# Los\_alfas(Integrantes del equipo)

Adrian\_Reig,Carlos Juan Battistini,Rodrigo\_Yamill.
...
3.2º Se copia el código fuente al contenedor.
3.3º Se instalan las dependencias desde el archivo requirements.txt.
3.4º Se descargan y generan los datos (json\_sp500.json y json\_ibex35.json) ejecutando los scripts de src/.
3.5º Se lanza la interfaz web mediante web/Inicio.py.
Instrucciones:
...
Los datos (json\_sp500.json y json\_ibex35.json) no se suben a git porque pesan mucho. Se descargan y generan dentro del contenedor durante el build, por eso el primer "docker build" tarda un rato.
