...
all_stats = {**calculate_stock_stats(sp500_data), **ibex35_stats}
...
<div style='background: #001a33; padding: 25px; border-radius: 10px; border-top: 3px solid #00aaff; margin-bottom: 20px;'>
    <div style='display: flex; align-items: center; gap: 12px;'>
        <h1 style='color: white; margin: 0; font-size: 44px;'>🇪🇸 IBEX 35</h1>
        <div style='color: {percent_color}; font-size: 22px; font-weight: bold;'>{sign}{percent:.2f}%</div>
    </div>
    <div style='color: #8899ac; font-size: 14px; margin-top: 6px;'>Índice Bursátil Español — {len(ibex35_data)} empresas</div>
</div>
...
search = st.text_input("", placeholder="Buscar símbolo...", label_visibility="collapsed", key="ibex_search")
...
filtered = {k: v for k, v in ibex35_stats.items() if search.upper() in k} if search else ibex35_stats
...
for symbol in sorted(filtered.keys()):
    ...
    if row[4].button("→", key=f"ibex_{symbol}"):
        st.session_state.ibex_stock = symbol
        st.rerun()
